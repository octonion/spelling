...
words = con.exec_params("select word from scripps.results where year=2015 and word not in (select distinct word from scripps.results where year<=2014) order by word asc;")
...
results = CSV.open("csv/words_2015.csv","w")
